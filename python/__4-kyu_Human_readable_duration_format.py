...
def _():
    ...
    def format_duration(seconds:float):
        ...
        def get_time(current_seconds,time):
            result, current_seconds = divmod(current_seconds,time)
        ...
        def create_stringformat():
            ...
            if uur > 1:
                result_string +=f"{int(uur)} hours, " if minuten > 0 else f"{int(uur)} hours"
            elif uur == 1:
                result_string +=f"{int(uur)} hour, " if minuten > 0 else f"{int(uur)} hour"

            if minuten > 1:
                if current_seconds == 0 and len(result_string) > 0:
                    result_string +=f"and {int(minuten)} minutes "
                else:
                    result_string +=f"{int(minuten)} minutes " if len(result_string) > 0 else f"{int(minuten)} minutes"
            elif minuten == 1:
                result_string +=f"{int(minuten)} minute " if len(result_string) > 0 else f"{int(minuten)} minute "

            if current_seconds > 1:
                result_string += f"and {int(current_seconds)} seconds" if len(result_string) > 0 else f"{int(current_seconds)} seconds"
            elif current_seconds == 1:
                result_string += f"and {int(current_seconds)} second" if len(result_string) > 0 else f"{int(current_seconds)} second"
            return result_string

        if current_seconds == 0: return "now"
